...
// ── CALC: totals update + preview refresh ──
function calc() {
  var fee  = g('fee');
  var fine = g('fine');
  var prev = g('prev');
...
// ── UPDATE: re-render receipt preview ──
function update() {
  if (!hasData()) {
  ...
  // read fresh values every time update() runs
  var fee  = g('fee');
  var fine = g('fine');
  var prev = g('prev');
  var conc = g('conc');
  var paid = g('paid');          // ← always fresh read here
  var total = fee + fine + prev - conc;
  var dues  = total - paid;
  ...
  var words = 'Rupees ' + n2w(Math.round(paid)) + ' Only.';   // uses fresh paid
